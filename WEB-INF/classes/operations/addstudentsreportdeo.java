...
public class addstudentsreportdeo {
	public int select(DataManager db,int year,int sem,String sec,addstudentsdeo ad,String branch)
	{
		...
		String query="select s.registerno from student_master.financestudent s where s.cyear="+year+" and s.semester="+sem+" and s.sectioncode='"+sec+"' and s.branchcode='"+branch+"' and s.registerno not in (select s1.registerno from student_master.student_faculty1 s1)" ;                                   
	...
	public int select1(DataManager db,int year,int sem,String sec,addstudentsdeo ad,String branch)
	{
		...
		String query="select s.registerno from student_master.financestudent s where s.cyear="+year+" and s.semester="+sem+" and s.sectioncode='"+sec+"' and s.branchcode='"+branch+"'";                                   
		System.out.println(query);
		boolean td=db.operation(query,"select");
	...
	{   
		...
		while(len>0)
		{
			...
			String query="insert into student_master.student_faculty1 values('"+reg[i]+"','"+fac+"')";
	...
	public int update(DataManager db,String reg[],String fac)
	{
		...
		while(len>0)
		{
			String query="update student_master.student_faculty1 set empcode='"+fac+"' where registerno='"+reg[i]+"'";
